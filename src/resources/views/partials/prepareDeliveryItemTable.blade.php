<table class="prepare-delivery-item-table table table-striped table-hover">
    <thead>
    <tr>
        <th>{{trans('allianceindustry::ai-deliveries.items.headers.type')}}</th>
        <th>{{trans('allianceindustry::ai-deliveries.items.headers.available_quantity')}}</th>
        <th>{{trans('allianceindustry::ai-deliveries.items.headers.selected_quantity')}}</th>
    </tr>
    </thead>
    <tbody>
    @foreach($items as $item)
        <tr>
            ...
            <td class="text-center" data-sort="{{$item->quantity}}">
                {{\RecursiveTree\Seat\AllianceIndustry\Helpers\AllianceIndustryHelper::formatNumber($item->availableQuantity(), 0)}}
            </td>
